...
from treemenusplus.utils import move_item, clean_ranks, move_item_or_clean_ranks

class TreemenusTestCase(TestCase):
    urls = 'treemenusplus.tests.urls'

    def setUp(self):
        # Install testapp
        self.old_INSTALLED_APPS = settings.INSTALLED_APPS
        settings.INSTALLED_APPS += ['treemenusplus.tests.fake_menu_extension']
        load_app('treemenusplus.tests.fake_menu_extension')
    ...
    def test_view_add_item(self):
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/add/', menu_data)
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/')
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/add/' % menu.pk, menu_item_data)
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/' % menu.pk)
        ...
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/items/%s/' % (menu.pk, new_menu_item.pk))
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/add/' % menu.pk, menu_item_data)
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/items/add/' % menu.pk)
    ...
    def test_view_history_item(self):
        ...
        response = self.client.get('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/history/' % (menu.pk, menu_item.pk))
    ...
    def test_view_delete_item(self):
        ...
        # Delete item confirmation
        response = self.client.get('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/delete/' % (menu.pk, menu_item.pk))
        self.assertEquals(response.request['PATH_INFO'], '/test_treemenus_admin/treemenusplus/menu/%s/items/%s/delete/' % (menu.pk, menu_item.pk))

        # Delete item for good
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/delete/' % (menu.pk, menu_item.pk), {'post': 'yes'})
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/' % menu.pk)
    ...
    def test_view_change_item(self):
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/' % (menu.pk, menu_item.pk), menu_item_data)
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/' % menu.pk)
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/' % (menu.pk, menu_item.pk), menu_item_data)
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/items/%s/' % (menu.pk, menu_item.pk))
    ...
    def test_move_up(self):
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/move_up/' % (menu.pk, menu_item3.pk))
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/' % menu.pk)
    ...
    def test_move_down(self):
        ...
        response = self.client.post('/test_treemenus_admin/treemenusplus/menu/%s/items/%s/move_down/' % (menu.pk, menu_item3.pk))
        self.assertRedirects(response, '/test_treemenus_admin/treemenusplus/menu/%s/' % menu.pk)
